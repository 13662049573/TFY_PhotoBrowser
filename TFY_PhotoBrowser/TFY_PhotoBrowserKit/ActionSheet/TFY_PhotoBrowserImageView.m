...
@interface TFY_PhotoBrowserImageView ()<UIScrollViewDelegate>{
    NSURL         *_url;
    UIImage       *_placeHolder;
}
@end

@implementation TFY_PhotoBrowserImageView

- (SDAnimatedImageView *)imageView{
    if (!_imageView) {
        _imageView = [[SDAnimatedImageView alloc] init];
        [_imageView setUserInteractionEnabled:true];
        [_imageView setFrame:CGRectMake(0, 0, PhotoWidth, PhotoHeight)];
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        [_scrollView setFrame:CGRectMake(0, 0, PhotoWidth, PhotoHeight)];
