...
@interface TFY_PhotoVideoCell ()<PhotoPlayerViewDelegate>
@end

@implementation TFY_PhotoVideoCell

- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        TFY_PhotoAVPlayerView *playerView = [[TFY_PhotoAVPlayerView alloc] initWithFrame:[UIScreen mainScreen].bounds];
        [playerView setDelegate:self];
        [self.contentView addSubview:playerView];
        _onlinePlayerView = playerView;
        
        TFY_PhotoLocateAVPlayerView *locatePlayerView = [[TFY_PhotoLocateAVPlayerView alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)playerLocatePhotoItems:(TFY_PhotoItems *)photoItems placeHolder:(UIImage *)placeHolder {
